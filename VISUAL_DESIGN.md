...
**Status:** active, dormant in this room (see Project Application)  
...
This room's product is plain terminal text: `node tour.mjs` prints the room map with no color, no icons, and no layout beyond indentation. Nothing below applies until someone adds a visual surface, which `BLUEPRINT.md` -> Non-Goals says will not happen in this generation. The reference is kept, unchanged apart from this section, because the v1.1 README lists it as a shipped control and `AGENTS.md` -> Day-One Checklist routes any UI task to it. Two rules still bind the terminal output today: state is never encoded with color alone (the tour and its test use text labels such as `owns`, `why`, `pass`, and `fail`), and emoji are not used as icons.
...
- Meet WCAG AA: body text at least 4.5:1 against its background; large text, icons, and UI strokes at least 3:1. `#F7F7F7` on `#0A0A0A` and `#191919` passes. Verify any text-on-accent or accent-on-dark pairing before shipping; several accents above fail AA for body text and are intended for fills, borders, and status, not small text.
